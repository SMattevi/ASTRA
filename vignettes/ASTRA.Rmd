...
# library(maditr) 
# msigdbr_collections() 
#msigdb_gene_sets = msigdbr(species = "human", category = "C5",subcategory = "MF") 
#  
# df_ann<-merge(df_PHASED,msigdb_gene_sets[,c("human_ensembl_gene","gs_name")],by.x=c("ensembl_gene_id"),by.y=c("human_ensembl_gene")) 
# df_ann<-unique(df_ann[,c("ensembl_gene_id","type","gs_name")]) 
#  
# gs<-merge(dcast(df_ann,gs_name~type)%>%top_n(10,Biallelic)%>%select(gs_name), 
#   dcast(df_ann,gs_name~type)%>%top_n(10,Monoallelic)%>%select(gs_name),all=TRUE) 
#  
# df_ann_plot<-df_ann[df_ann$gs_name%in%gs$gs_name,] 
# plot_annotation(df=df_ann_plot,ASC="type",gene="ensembl_gene_id",annotation="gs_name") 
#  
#m_t2g<-msigdb_gene_sets%>%dplyr::select(gs_name, gene_symbol) # head(m_t2g)


#enr<-enricher(unique(df_PHASED$hgnc_symbol[df_PHASED$type=="Monoallelic"]),TERM2GENE=m_t2g, universe=unique(df_PHASED$hgnc_symbol),pvalueCutoff = 0.2,qvalueCutoff = 0.2)  
#head(enr)
```
...
```{r}
#expr<-read_salmon(salmon_path=system.file("extdata", "quant.sf", package = "ASTRA"),txdb=EnsDb.Hsapiens.v86,counts=0)

seesaw(names = sample, edb_id="AH113665",tss_gap=50,gene_id="ENSG00000214548",files="/mnt/projects-2024/ASTRA/ASTRA_bulk_pipeline/results/seesaw/salmon/quant.sf",cellline_id="bj")
```
